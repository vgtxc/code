...
"""
import os,shutil,re,random,argparse,hashlib,time
class Hanime1MeTool():
	...
	def subString(self,string="",rep=""):	# 批量删除文件中的指定字符串
		for file in self.filePathDict.keys():
			path = self.filePathDict[file]
			old = os.path.join(path,file)
			new = os.path.join(path,re.sub(r""+string,rep,file))
			shutil.move(old,new)
			print("subString...---->",new)
...
# 保存文件夹的路径到文件
def saveDirName2Txt(dirpath):
	dircont = os.listdir(dirpath)
	dircont.remove("subtitle")
...
if __name__ == "__main__":
	srcPath = "D:/entertain/video/[小余]小玉儿"
