...
//1)base->打印链表一个节点
void printNode(const struct listNode *node){
    if(node==NULL){printf("->%p is empty!!!\n",node);}
    if(node!=NULL){printf("->%p\t%d\t%s\t%s\n",node,node->index,node->key,node->valu);}
}
...
//1)链表初始化,创建一个链表节点
void *listInit(int *index,char *key,char *valu){
...
void *copyNode(struct listNode *node){
    ...
    return listInit(&node->index,node->key,node->valu);
}
...
void *copyList(struct listNode *list){
    ...
    for(struct listNode *cur=list;cur!=NULL;cur=cur->behind){
        struct listNode *newnode = listInit(0,cur->key,cur->valu);
...
//7.1)释放空间,及指针
void *freeList(struct listNode *list){
    char *rpl = stringCopy(NULL);
    for(;list!=NULL;list=list->behind){
        free(list->front);
        if(list->behind==NULL){free(list);}
    }
    return list;
}
